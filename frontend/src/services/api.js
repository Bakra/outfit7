...
export const eventsApi = {
  ...
  async getEvents() {
    ...
    try {
      ...
      console.log("API: Processed events array:", events);
      return events;
    } catch (error) {
      ...
      if (error.response) {
        ...
        console.error(
          "API: Server responded with error:",
          error.response.status,
          error.response.data
        );
        throw new Error(
        ...
        throw new Error(
          "No response from server. Check if backend is running on http://localhost:3000"
        );
      } else {
        // Something else happened - this is what the test expects
